...
const BentoGrid = ({ children, className, ...props }: BentoGridProps): ReactElement => {
  return (
    <div
      className={cn(
        'grid w-full auto-rows-[22rem] grid-cols-4 gap-6',
        className,
      )}
      {...props}
    >
...
const BentoCard = ({
  ...
  >
    ...
    <div className='pointer-events-none z-10 flex transform-gpu flex-col gap-1 p-6 transition-all duration-300 group-hover:-translate-y-10'>
      <Icon className='text-primary-foreground h-12 w-12 origin-left transform-gpu transition-all duration-300 ease-in-out group-hover:scale-75' />
      <h3 className='text-primary-foreground dark:text-primary-foreground text-xl font-semibold'>
        {name}
      </h3>
      <p className='text-primary-foreground max-w-lg'>{description}</p>
    </div>
